...
class MinhaTela:
        ...
        try:
            ...
            while True:
                for event in pygame.event.get():
                    ...
                    tempoMaximo = 5
                    # se o usuário clicar no personagem, ele vai ser exibido no canto esquerdo da tela
                    if event.type == MOUSEBUTTONDOWN:
                        for i in range(tempoMaximo + 1):
                            sleep(1)
                            print(self.tempo_de_escolha(tempoMaximo))
                            tempoMaximo -= 1
                            ...
                            '''
                            #relogio sera exibido no meio da tela
                            fonte = pygame.font.Font("fontes/font.ttf",30)
                            textoTempo = fonte.render(self.tempo_de_escolha(tempoMaximo), True,(0,0,0))
                            pygame.display.update(self.tela.blit(textoTempo,[400,5]))
    ...
    #relogio
    def tempo_de_escolha(self, x):
        try:
            minuto = int(x / 60)
            segundo_rem = int(x % 60)
            if segundo_rem < 10:
                return str(minuto) + ":0" + str(segundo_rem)
            else:
                return str(minuto) + ":" + str(segundo_rem)
        except Exception, e:
            print (e)
